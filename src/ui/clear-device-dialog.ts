...
interface ClearDeviceDependencies {
  ...
  browserStorage: Pick<Storage, "setItem">;
}
...
export function renderClearDeviceDialog(open: boolean): string {
  ...
  return `<div class="confirm-overlay"><section class="confirm-dialog" role="alertdialog" aria-modal="true" aria-labelledby="clear-device-title" aria-describedby="clear-device-description" data-clear-device-dialog><div class="confirm-dialog__icon" aria-hidden="true">!</div><div><p class="eyebrow">Confirmación necesaria</p><h2 id="clear-device-title">¿Borrar los datos de este dispositivo?</h2><p id="clear-device-description" class="muted">Se eliminarán la colección, Wants y el historial local. Solo podrás recuperarlos si antes exportaste una copia.</p></div><div class="confirm-dialog__actions"><button class="button button--quiet" type="button" data-action="cancel-clear-device">Cancelar</button><button class="button button--danger" type="button" data-action="confirm-clear-device">Sí, borrar datos locales</button></div></section></div>`;
}
