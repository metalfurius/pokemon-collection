...
Product media has its own IndexedDB schema and archive manifest version. It is keyed by stable local record id (`record:<id>`) so later Cardmarket enrichment does not orphan a photo. A full backup ZIP must validate its collection envelope, media manifest, declared byte sizes, WebP signatures, paths, keys, file count, and expanded size before replacing either store. A future media schema requires an explicit asset migration and full-backup parser branch; never silently reinterpret a newer manifest. JSON restores intentionally preserve the current local gallery, while full ZIP restores replace it.
